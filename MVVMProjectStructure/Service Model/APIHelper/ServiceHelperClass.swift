...
class ServiceHelperClass: NSObject {
    ...
    func addInstagramAccount2(username:String,password:String,completionHandler: @escaping(Any?, Error?) -> Void) -> Void{
        
        let url = URL(string: "\(Configuration.BASE_URL)add-account")
        
        let bodyString = "username=\(username)&password=\(password)&device_id=\(Configuration.DEVICE_ID)&device_type=\(Configuration.DEVICE_TYPE)"

        postRequest(with:url!, postBody: bodyString) { (response) in

            completionHandler(response as Any,nil)
        }
    }
    ...
    func postRequest(with url:URL, postBody:String, callback: @escaping (Any?) -> Void) -> Void {
        ...
        session.dataTask(with: urlRequest, completionHandler: { (data, urlResponse, error) in
            ...
            if httpResponse.statusCode == 200 {
                ...
                do {
                    guard let responseData = try JSONSerialization.jsonObject(with: responseData, options: JSONSerialization.ReadingOptions.mutableLeaves) as? [String: AnyObject] else {
